...
def batch_divide(func, n, batch_size=1):
    ...
    for i in range(times):
        res.append(
            func(slice(start + i * batch_size, start + (i + 1) * batch_size)))
...
class Ssim(object):
    ...
    def flows(cls, imgs):
        ...
        imgs = torch.tensor(
            np.transpose(
                imgs.resize(
                    cls.size), (0, 3, 1, 2)), dtype=torch.float32)
        n = len(imgs)
        ssim_flows = torch.zeros((n, n), dtype=torch.float32)
        for i in range(n - 1):
            ssim_flows[i, i + 1:] = 1 - \
                cls.score(imgs[i:i + 1].expand(n - i - 1, -1, -1, -1), imgs[i + 1:])
...
class Lightglue(object):
    ...
    def match(cls, p1s, p2s):
        ...
        k2s = p2s['keypoints']
        with torch.inference_mode():
            _, indices = cls.matcher(
                p1s['descriptors'], p2s['descriptors'], KF.laf_from_center_scale_ori(
                    k1s[None]), KF.laf_from_center_scale_ori(
                    k2s[None]))
...
class Aliked_LightGlue(object):
            ...
            torch.tensor(
                np.transpose(
                    ...
                    (2,
                     0,
                     1)),
                dtype=torch.float32) for i in imgs.resize(
                cls.sizes[size_i])]
    ...
    @classmethod
    def rot(cls, imgs):
        imgs4 = [[torch.rot90(i, rot, (1, 2)) for rot in range(4)]
                 for i in cls.resize(imgs, 0)]
    ...
    def flows(cls, imgs):
        ...
        imgs3 = [[torch.rot90(imgs3[i][j], rots[j], (1, 2))
                  for i in range(len(cls.sizes))] for j in range(n)]
        res = torch.zeros((n, n), dtype=torch.float32)
        for i in range(n):
            res[i, i] = 0
            for j in range(i + 1, n):
                v = sum([Lightglue.match(cls.extract(i1), cls.extract(i2))
                        for i1, i2 in zip(imgs3[i], imgs3[j])])
                res[i, j] = int(1e2 / v)
                res[j, i] = res[i, j]
        return res
...
def main():
    ...
    for scene in scenes:
        ...
        if category.transparent(scene):
            ...
            distance_matrix = np.int32((ssim_flows * 1e6 + matching_flows))
